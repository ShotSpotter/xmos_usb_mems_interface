...
int hotplug_cb(libusb_context *ctx, libusb_device *device, libusb_hotplug_event event, void *user_data) {
    printf("DFU device plugged on bus %d, dev %d\n",
            ...
            if (found == id)
            {
                ...
                else
                {
                    ...
                    libusb_hotplug_register_callback(
                        ...
                        LIBUSB_HOTPLUG_NO_FLAGS, desc.idVendor, desc.idProduct,
...
int write_dfu_image(char *file)
{
    ...
    for (i = 0; i < num_blocks; i++)
    {
        ...
        printf("... Downloading block number (%04d/%d) size %d\r", dfuBlockCount, num_blocks, block_size);
...
//#define START_IN_DFU 1
    ...
    else if(!listdev)
    {
        ...
        while (dev == NULL) {
            ...
            struct timespec ts  = { 0, 10000000UL };
            nanosleep(&ts, NULL);
        }
#endif
