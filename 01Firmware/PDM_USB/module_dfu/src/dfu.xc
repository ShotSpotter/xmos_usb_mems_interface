...
int DFUDeviceRequests(XUD_ep ep0_out, XUD_ep &?ep0_in, USB_SetupPacket_t &sp, chanend ?c_user_cmd, unsigned int altInterface, client interface i_dfu i,int &reset) {
	...
	printstrln("g_DFU_state #1");
	printintln(g_DFU_state);
	...
	printstrln("g_DFU_state #2");
	printintln(g_DFU_state);
	printintln(data_buffer[0]);
	printintln(data_buffer[1]);
	printintln(data_buffer[2]);
	printintln(data_buffer[3]);
